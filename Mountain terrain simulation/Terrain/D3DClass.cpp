...
bool D3DClass::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	if (FAILED(m_device->CreateRasterizerState(&rasterDesc, &m_rasterStateWireframe))) {
		return false;
	}
	...
	if (FAILED(m_device->CreateBlendState(&blendStateDescription, &m_alphaDisalbeBlendingState))) {
		return false;
	}
