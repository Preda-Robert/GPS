...
void TerrainCellClass::GetCellDimensions(float& maxWidth, float& maxHeight, float& maxDepth, float& minWidth, float& minHeight, float& minDepth)
{
	maxWidth = m_maxWidth;
...
bool TerrainCellClass::BuildLineBuffers(ID3D11Device* device)
{
	...
	if (FAILED(device->CreateBuffer(&vertexBufferDesc, &vertexData, &m_lineVertexBuffer)) ||
		FAILED(device->CreateBuffer(&indexBufferDesc, &indexData, &m_lineIndexBuffer))
	) {
		return false;
	}
